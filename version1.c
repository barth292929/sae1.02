...
char calcTraj(tPlateau plateau, int lesX[], int lesY[], int nb_pomme);
...
int main(){
	...
	do 
	{
		direction = calcTraj(lePlateau, lesX, lesY, nbPommes); 
...
char calcTraj(tPlateau plateau, int lesX[], int lesY[], int nb_pomme){
	if (lesX[0] < lesPommesX[nb_pomme]){  //si la pomme est plus à droite que le serpent, il va vers la droite
		return DROITE;
	}
	else if (lesX[0] > lesPommesX[nb_pomme]){  //si la pomme est plus à gauche que le serpent, il va vers la gauche
		return GAUCHE;
	}
	if (lesY[0] > lesPommesY[nb_pomme]){  //si la pomme est plus en haut que le serpent, il va vers le haut
		return HAUT;
	}
	else if (lesY[0] < lesPommesY[nb_pomme]){  //si la pomme est plus en bas que le serpent, il va vers le bas
		return BAS;
	}
	return DROITE;
}
